...
export default function useFoodManagement() {
  ...
  async function saveFood() {
    ...
    const response = await axios.post(API_ENDPOINTS.ADD_FOOD_ITEM, {
    ...
    if (response.data.success) {
      ...
      foodAdd.value.categoryId = 0;
      foodAdd.value.foodName = "";
      foodAdd.value.priceCustom = "";
      foodAdd.value.priceListed = "";
      foodAdd.value.unit = "";
    } else {
  ...
  async function confirmUpdateFoodItem(foodItemCurrentUpdate) {
    ...
    console.log("Food Current Update ID", foodItemCurrentUpdate.categoryId);

    // const categoryId = foodItemCurrentUpdate.categoryId.split("(")[0];
    ...
    if (response.data.success) {
      modalUpdateFoodItem.value = !modalUpdateFoodItem.value; // Đóng modal
  ...
  function updateCurrentFoodSelected(item) {
    ...
    // Cập nhật danh sách món đã ch���n
